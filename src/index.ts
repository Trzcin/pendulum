...
const a1Input = document.getElementById('a1') as HTMLInputElement;
...
const a2Input = document.getElementById('a2') as HTMLInputElement;
...
const bSInput = document.getElementById('bs') as HTMLInputElement;
bSInput.value = massSizeMultiplyer.toString();
bSInput.onchange = () => (massSizeMultiplyer = parseFloat(bSInput.value));

const lWInput = document.getElementById('lw') as HTMLInputElement;
...
function updatePlayIcon() {
  if (paused) {
    (playBtn.children[0] as HTMLImageElement).src = './icons/play.svg';
  } else {
    (playBtn.children[0] as HTMLImageElement).src = './icons/pause.svg';
  }
}
...
function calculateAcceleration() {
  const acc1_p1 =
    ...
    2 *
      Math.sin(a1 - a2) *
      m2 *
      (Math.pow(a2_v, 2) * r2 + Math.pow(a1_v, 2) * r1 * Math.cos(a1 - a2));
  ...
  const acc2_p1 =
    2 *
    Math.sin(a1 - a2) *
    (Math.pow(a1_v, 2) * r1 * (m1 + m2) +
      g * (m1 + m2) * Math.cos(a1) +
      Math.pow(a2_v, 2) * r2 * m2 * Math.cos(a1 - a2));
